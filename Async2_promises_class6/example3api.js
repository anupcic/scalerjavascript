...
// const myFetch = new Promise(function (resolve, reject) {
//   setTimeout(function () {
//     resolve(data);
//   }, 10000);
// });
...
const fetchToDoById = function (id) {
  return new Promise(function (resolve, response) {
    setTimeout(function () {
      const result = data.filter(function (val, idx) {
        return val.id == id;
      });
      if (result) {
        resolve(result);
      } else {
        reject("No task found with the given ID");
      }
    }, 10000);
  });
};

const fetchToDoById2 = function (id) {
  return new Promise(function (resolve, response) {
    setTimeout(function () {
      const result = data.find(function (fid) {
        return fid.id == id;
      });
      if (result) {
        resolve(result);
      } else {
        reject("No task found with the given ID", `${id}`);
      }
    }, 10000);
  });
};
fetchToDoById2("V59OF92YF62HFY0").then(function (data) {
  console.log(data);
});
